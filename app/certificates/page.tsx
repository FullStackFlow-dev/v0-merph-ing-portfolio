...
export default async function CertificatesPage() {
  ...
  // Organize by domain
  const certsByDomain = certificates?.reduce((acc: any, cert: any) => {
          ...
          {sortedDomains.map((domain) => (
            <section key={domain} className="mb-16 last:mb-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {certsByDomain[domain].map((cert: any) => (
                  <div
                    key={cert.id}
                    className="group relative h-full bg-gradient-to-br from-slate-700 to-slate-800 rounded-lg overflow-hidden hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-300 transform hover:-translate-y-1 border border-slate-600 hover:border-blue-500/50"
                  >
                    {/* Image Container */}
                    {cert.image_url && (
